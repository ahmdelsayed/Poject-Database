...
class BillClass:
    def __init__(self, root):
        ...
        self.title = Label(self.header, text=" Inventory Management System", image=self.icon_title, compound="left", font=("Segoe UI", 35, "bold"), bg="#010c48", fg="white", anchor="w", padx=20)
        self.title.pack(side=LEFT, fill=Y)

        btn_logout = Button(self.header, text="Logout", command=self.logout, font=("Segoe UI", 12, "bold"), bg="#E74C3C", fg="white", cursor="hand2", bd=0, padx=20, pady=5)
        ...
        txt_search = Entry(ProductFrame1, textvariable=self.var_search, font=("Segoe UI", 13), bg="#ebf5fb", bd=1)
        txt_search.place(x=10, y=10, width=190, height=35)
        ...
        Entry(CustomerFrame, textvariable=self.var_cname, font=("Segoe UI", 11), bg="#f9f9f9").place(x=70, y=10, width=150)
        ...
        r, c = 1, 0
        for b in btns:
            cmd = lambda x=b: self.get_input(x)
            if b == 'C': cmd = self.clear_cal
            elif b == '=': cmd = self.perform_cal
            Button(Cal_Frame, text=b, font=("Segoe UI", 14, "bold"), width=4, pady=10, bg="#34495e", fg="white", cursor="hand2", bd=0, command=cmd).grid(row=r, column=c, padx=2, pady=2)
            c+=1
            if c>3: c=0; r+=1
        ...
        btn_clear_all = Button(BillMenuFrame, text="CLEAR ALL", command=self.clear_all, font=("Segoe UI", 11, "bold"), bg="#64748b", fg="white", cursor="hand2", bd=0).place(x=0, y=75, width=120, height=45)
        btn_generate = Button(BillMenuFrame, text="GENERATE", command=self.generate_bill, font=("Segoe UI", 11, "bold"), bg="#0f172a", fg="white", cursor="hand2", bd=0).place(x=125, y=75, width=120, height=45)
        btn_print = Button(BillMenuFrame, text="PRINT", command=self.print_bill, font=("Segoe UI", 11, "bold"), bg="#3b82f6", fg="white", cursor="hand2", bd=0).place(x=250, y=75, width=125, height=45)
    ...
    def get_data(self, ev):
        ...
        price_val = "".join(filter(lambda x: x.isdigit() or x=='.', str(row[2])))
    ...
    def add_update_cart(self):
        if self.var_pid.get() == '': messagebox.showerror('Error', "Select product", parent=self.root)
            ...
            for row in self.cart_list:
                ...
                index_ += 1
            if present == 'yes':
                op = messagebox.askyesno('Confirm', "Already in cart. Update or Remove?", parent=self.root)
                if op:
                    if self.var_qty.get() == "0": self.cart_list.pop(index_)
                    else: self.cart_list[index_][2] = price_cal; self.cart_list[index_][3] = self.var_qty.get() 
    ...
    def update_stock(self):
        ...
        try:
            for row in self.cart_list:
                new_qty = int(row[4]) - int(row[3])
                status = 'Active' if new_qty > 0 else 'Inactive'
                cur.execute("update product set qty=?, status=? where pid=?", (new_qty, status, row[0]))
                con.commit()
            self.show()
    ...
    def perform_cal(self):
        ...
        except: messagebox.showerror("Error", "Invalid calc")
